...
static char *errmsg[] = {
/*001*/  "Ожидалось: \"%s\", но найден \"%s\"\n",
/*002*/  "за каждым из них может следовать только один оператор (или выражение). \"case\"\n",
/*003*/  "объявление локальной переменной должно отображаться в составном блоке\n",
/*004*/  "функция \"%s\" не реализована %s\n",
/*005*/  "функция не имеет аргументов\n",
/*006*/  "должно быть присвоено к массиву\n",
/*007*/  "оператор не может быть переопределен\n",
/*008*/  "должно быть постоянным выражением; вернуло ноль\n",
/*009*/  "невалидный размер массива (меньше нуля, ноль или выход за пределы)\n",
/*010*/  "неизвестная функция\n",
/*011*/  "смещение стека/адрес данных должны быть кратны размеру ячейки\n",
/*012*/  "невалидный вызов функции, не валидный адрес\n",
/*013*/  "нет точки входа. (нет функций) тспользуйте main(){}\n",
/*014*/  "неверное утверждение; отсутствует в switch\n",
/*015*/  "\"default\" case должен быть последним регистром в операторе switch\n",
/*016*/  "несколько значений по умолчанию в \"switch\"\n",
/*017*/  "не найден символ \"%s\"\n",
/*018*/  "данные инициализации превышают заявленный размер\n",
/*019*/  "это не ярлык: \"%s\"\n",
/*020*/  "Не валидное имя символа \"%s\"\n",
/*021*/  "Символ уже определён: \"%s\"\n",
/*022*/  "должно быть значение lvalue (непостоянное)\n",
/*023*/  "назначение массива должно быть простым\n",
/*024*/  "\"прервать\"или \"продолжить\" убраны из контекста\n",
/*025*/  "заголовок функции отличается от прототипа\n",
/*026*/  "нет совпадений \"%s\"\n",
/*027*/  "недопустимая символьная константа\n",
/*028*/  "недопустимый нижний индекс (не массив или слишком много нижних индексов): \"%s\"\n",
/*029*/  "неверное выражение, предполагается ноль\n",
/*030*/  "составной оператор не закрыт в конце файла (начало в строке %d)\n",
/*031*/  "неизвестная директива\n",
/*032*/  "индекс массива вне границ (переменная \"%s\")\n",
/*033*/  "должен быть индекс массива (переменная \"%s\")\n",
...
/*039*/  "константа символа не имеет размера\n",
/*040*/  "дублирование метки \"case\" (значение %d)\n",
/*041*/  "неверное многоточие, размер массива неизвестен\n",
/*042*/  "неверное сочетание классификаторов\n",
/*043*/  "константа символа превышает диапазон для упакованной строки\n",
...
/*051*/  "неверный индекс, используйте операторы \"[ ]\" для основных измерений\n",
...
/*056*/  "массивы, локальные переменные, аргументы функций и определенные пользователем операторы не могут быть публичными (символ \"%s\")\n",
/*057*/  "незавершенное выражение перед директивой компилятора\n",
...
/*063*/  "результат функции оператора \"%s\" должен быть \"%s\"\n",
/*064*/  "нельзя изменять предопределенные операторы\n",
...
/*071*/  "определенный пользователем оператор должен быть объявлен перед использованием (функция \"%s\")\n",
/*072*/  "оператор \"sizeof\" недействителен для символов \"function\"\n",
...
/*080*/  "неизвестный символ или не константа символа (символ \"%s\")\n",
/*081*/  "нельзя использовать тег в качестве значения по умолчанию для параметра индексированного массива (символ \"%s\")\n",
/*082*/  "определенные пользователем операторы и нативные функции не могут иметь состояний\n",
...
/*093*/  "оператор \"__addressof\" недействителен в выражениях препроцессора\n",
/*094*/  "деление на ноль\n"

};

static char *fatalmsg[] = {
  /*100*/  "не удается прочитать файл: \"%s\"\n",
  /*101*/  "не удается записать в файл: \"%s\"\n",
  /*102*/  "переполнение таблицы: \"%s\"\n",
  /* таблица может быть: таблицей циклов
   *                     таблицей литералов
   *                     буфером постановки
   *                     таблицей опций (файл ответов)
   *                     таблицей оптимизатора пипелайна
   */
...
static char *warnmsg[] = {
  ...
  /*201*/  "переопределение константы/макроса (символ \"%s\")\n",
  ...
  /*208*/  "функция с результатом в виде тега использована до определения, что вынуждает повторный разбор\n",
  ...
  /*214*/  "возможно, предполагался аргумент массива \"const\": \"%s\"\n",
  ...
  /*224*/  "неопределенный размер массива в выражении \"sizeof\" (символ \"%s\")\n",
  /*225*/  "недостижимый код\n",
  /*226*/  "переменной присвоено ее же значение (символ \"%s\")\n",
  ...
  /*236*/  "неизвестный параметр в подстановке (некорректный шаблон #define)\n",
  ...
  /*240*/  "присвоенное значение никогда не используется (символ \"%s\")\n",
  /*241*/  "отрицательное или слишком большое количество сдвигов\n",
  /*242*/  "переполнение сдвига при объявлении элемента перечисления (символ \"%s\")\n",
  /*243*/  "избыточный код: выражение управления переключателем является постоянным\n",
  /*244*/  "элемент перечисления \"%s\" не обработан в переключателе\n",
  /*245*/  "приращение перечисления \"%s %d\" не влияет на нулевое значение (символ \"%s\")\n",
  /*246*/  "переполнение умножения при объявлении элемента перечисления (символ \"%s\")\n",
  /*247*/  "использование оператора \"%s\" на %s\n",
  /*248*/  "возможное неправильное использование оператора запятой\n",
  /*249*/  "проверка не пройдена: %s\n",
  /*250*/  "переменная \"%s\", используемая в условии цикла, не изменяется в теле цикла\n",
  /*251*/  "ни одна из переменных, используемых в условии цикла, не изменяется в теле цикла\n",
  /*252*/  "переменная изменяет свое значение, но оно никогда не используется: \"%s\"\n"

};

static char *noticemsg[] = {
/*001*/  "; Может ты имел ввиду \"%s\"?\n",
/*002*/  "; переменная вне области видимости\n",
...
{
  static char *prefix[3]={ "Ошибка", "Критическая ошибка", "Предупреждение" };
